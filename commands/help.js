...
module.exports = {
    ...
    description: "Показывает все доступные комманды",
    usage: "help [command]",
    ...
    const embed = new Discord.MessageEmbed()
        ...
        embed.addField(":grey_question: Информация комманд", `${prefix} помоги <комманда>`);
        if(config.news && config.newstitle) {
        embed.addField(`${config.newstitle}`, `${config.news}`);
        }
        embed.setFooter("Запрошено: " + `${message.author.username}` + " • " + `${client.commands.size}` + " команды", message.author.displayAvatarURL({ dynamic: true, format: 'png', size: 2048 }))
    ...
    message.channel.send({embed: {
        color: 16734039,
        description: "Что-то произошло не так... :cry:"
    }})
    }
    }
}